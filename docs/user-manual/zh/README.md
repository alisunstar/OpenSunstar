...
- X 轴：提交活跃度
- Y 轴：已配 AI 且算出健康分时画 **AI 健康评分**，否则画 **Agent 配置就绪分**（整张图只用一种，不混画）
- 气泡大小：代码规模
- 一图纵览所有项目的相对位置
- **不需要 API Key** —— 这张图只画本地已有的数字，不调用任何模型
- 拿不到 Y 值的项目（未纳管 / 未扫描）不会出现在图上，不会被填一个占位坐标
...
> 项目组合矩阵与 Agent 配置就绪分**不在此列**——它们完全由本地扫描得出，未配置 AI 模型时同样可用。
...
- **自动更新** — 检查并安装应用内更新
- **备份管理** — 查看、创建、恢复备份
- **法律信息** — Apache-2.0 公共客户端许可证、第三方声明与私有云端控制面边界
