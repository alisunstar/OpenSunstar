...
App routing means letting OpenSunstar route a specific application's API requests through the local routing service.
...
When routing is enabled, OpenSunstar modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, OpenSunstar will:
...
Before enabling routing, OpenSunstar backs up the original configuration and restores it when disabled.
