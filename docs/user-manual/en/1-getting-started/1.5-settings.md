...
This section describes how to configure OpenSunstar according to your preferences.
...
OpenSunstar supports four languages:
...
When enabled, OpenSunstar automatically runs when the system starts.
...
Starting from v3.13.0, OpenSunstar adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger OpenSunstar via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If OpenSunstar runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, OpenSunstar automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that OpenSunstar uses when opening a terminal.
...
The storage location for OpenSunstar's own data, defaulting to `~/.OpenSunstar/`.
...
The exported file name format is `OpenSunstar-export-{timestamp}.sql`.
...
Configure OpenSunstar's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, OpenSunstar automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `OpenSunstar-sync`) |
...
- OpenSunstar automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, OpenSunstar displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current OpenSunstar version number, with support for:
