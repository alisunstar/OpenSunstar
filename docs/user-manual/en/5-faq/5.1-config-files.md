...
## OpenSunstar Data Storage

### Storage Directory

Default location: `~/.OpenSunstar/`
...
```
~/.OpenSunstar/
├── OpenSunstar.db      # SQLite database (SSOT)
...
`OpenSunstar.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. OpenSunstar writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to OpenSunstar's "providers") |
...
OpenSunstar's priority when modifying configurations:

1. **OpenSunstar Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by OpenSunstar)
- OpenSunstar's `settings.json`

### Not Recommended to Edit Manually

- `OpenSunstar.db` database file
- Backup files
...
OpenSunstar v3.7.0 migrated from JSON files to SQLite:
