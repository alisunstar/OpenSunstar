...
function PrototypePreview({
  ...
  const has = (name: string) =>
    components.some((component) =>
      component.toLowerCase().includes(name.toLowerCase()),
    );
  const showTable = has("table") || has("list") || index % 3 === 0;
  const showForm = has("form") || has("editor") || index % 3 === 2;
  const showSidebar =
    has("sidebar") || has("split") || has("drawer") || index % 3 === 1;
  ...
  return (
    <div
      style={tokenStyle}
      className="rounded-lg border border-[var(--proto-border)] bg-[var(--proto-surface)] p-[var(--proto-spacing)] text-[var(--proto-text)] shadow-inner"
      aria-label={`${page ?? "页面"} 静态原型`}
    >
      <div className="mb-3 flex items-center justify-between border-b border-[var(--proto-border)] pb-2">
        <div className="flex items-center gap-2">
          <div className="h-2.5 w-2.5 rounded-full bg-[var(--proto-primary)]" />
          <span className="text-[11px] font-semibold">
            {page ?? "页面模板"}
          </span>
        </div>
        <div className="flex gap-1">
          <span className="h-2 w-8 rounded bg-[var(--proto-muted)]/25" />
          <span className="h-2 w-5 rounded bg-[var(--proto-muted)]/15" />
        </div>
      </div>
      {showMetrics && (
        <div className="mb-3 grid grid-cols-3 gap-2">
          {["活跃用户", "转化率", "待处理"].map((label, i) => (
            <div
              key={label}
            ...
            >
              ...
              <div className="mt-2 h-1 rounded bg-primary/30">
                <div className="h-1 w-2/3 rounded bg-primary" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div
        className={cn(
          "grid gap-[var(--proto-spacing)]",
          showSidebar ? "grid-cols-[150px_1fr]" : "grid-cols-1",
        )}
      >
        {showSidebar && (
          <aside className="rounded-md border border-[var(--proto-border)] bg-[var(--proto-background)] p-2">
            <div className="mb-2 h-2 w-16 rounded bg-[var(--proto-muted)]/30" />
            <div className="space-y-1.5">
              {["总览", "项目列表", "成员与权限", "设置"].map((item, i) => (
                <div
                  key={item}
                  className={cn(
                    "rounded px-2 py-1.5 text-[9px]",
                    i === 0
                      ? "bg-[var(--proto-primary)]/15 text-[var(--proto-primary)]"
                      : "text-[var(--proto-muted)]",
                  )}
                >
                  {item}
                </div>
        ...
        <div className="space-y-3">
          {showTable && (
            <div className="overflow-hidden rounded-md border border-border/60 bg-background">
              <div className="flex items-center justify-between border-b border-border/60 p-2">
                ...
                <div className="h-6 w-16 rounded bg-primary/85" />
              </div>
              <div className="grid grid-cols-3 gap-2 border-b border-border/50 px-2 py-1.5 text-[8px] text-muted-foreground">
                <span>名称</span>
                <span>状态</span>
                <span>更新时间</span>
              </div>
              {["示例项目 A", "示例项目 B", "示例项目 C"].map((row) => (
                <div
                  key={row}
                  className="grid grid-cols-3 gap-2 border-b border-border/40 px-2 py-2 text-[9px] last:border-0"
                >
                  <span>{row}</span>
                  <span className="text-emerald-500">进行中</span>
          ...
          {showForm && (
            <div className="rounded-md border border-border/60 bg-background p-3">
              <div className="mb-3 h-2 w-24 rounded bg-foreground/20" />
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <span className="text-[8px] text-muted-foreground">名称</span>
                  <div className="h-7 rounded border border-border/70" />
                </div>
                <div className="space-y-1">
                  <span className="text-[8px] text-muted-foreground">状态</span>
                  <div className="h-7 rounded border border-border/70" />
                </div>
              </div>
              <div className="mt-2 space-y-1">
                <span className="text-[8px] text-muted-foreground">说明</span>
                <div className="h-12 rounded border border-border/70" />
              </div>
              <div className="mt-3 flex justify-end">
                <div className="h-7 w-16 rounded bg-primary/85" />
              </div>
            </div>
          )}
          {!showTable && !showForm && (
            <div className="rounded-md border border-border/60 bg-background p-4">
              ...
              <div className="mt-3 h-2 w-3/4 rounded bg-muted-foreground/20" />
              <div className="mt-2 h-2 w-1/2 rounded bg-muted-foreground/15" />
              <div className="mt-4 h-8 w-24 rounded bg-primary/85" />
            </div>
          )}
        </div>
      </div>
...
export default function ProjectDesignContractPanel({
  projectId,
}: {
  projectId: string;
}) {
  ...
  const [installResult, setInstallResult] =
    useState<DesignInstallResult | null>(null);
  const [importUrl, setImportUrl] = useState("");
  const [remoteImportKind, setRemoteImportKind] = useState<"github" | "shadcn">(
    "github",
  );
  const [importing, setImporting] = useState(false);
  const [installPlan, setInstallPlan] = useState<DesignInstallPlan | null>(
    null,
  );
  ...
  const [packageDetail, setPackageDetail] =
    useState<DesignSystemPackageDetail | null>(null);
  const [selectedPrototype, setSelectedPrototype] = useState<string | null>(
    null,
  );
  const [verification, setVerification] =
    useState<DesignSystemVerification | null>(null);
  ...
  useEffect(() => {
    designContractApi
      .listPackages()
      .then((result) => setPackages(result.packages))
      .catch(() => setPackages([]));
  }, []);

  const buildParams = useCallback(
    (): DesignContractParams => ({
      ...
      spacing: contract?.spacing
        ? { ...contract.spacing, baseUnit: spacingBase }
        : null,
  ...
  const handlePreview = useCallback(async () => {
    setPreviewing(true);
    try {
      setPreviewMd(await designContractApi.previewDesignMd(buildParams()));
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        t("designContract.copied", { defaultValue: "Copied to clipboard" }),
      );
    } catch {
  ...
  const handleExportPreview = useCallback(async () => {
    ...
    try {
      setExportPlan(
        await designContractApi.previewExportPlan(projectId, buildParams()),
      );
    } catch (e) {
      toast.error(String(e));
    } finally {
  ...
  const handleExportConfirm = useCallback(async () => {
    ...
    setExporting(true);
    try {
      const p = await designContractApi.exportContract(
        projectId,
        buildParams(),
      );
      toast.success(
        t("designContract.exported", {
          defaultValue: "已覆盖导出设计规范与 Tokens",
        }),
      );
      setPreviewMd(p);
      setVerification(await designContractApi.verifyProjectSystem(projectId));
  ...
  const handleInstallPreview = useCallback(async () => {
    setInstalling(true);
    try {
      const plan = await designContractApi.previewInstallPlan(
        projectId,
        buildParams(),
      );
      setInstallPlan(plan);
  ...
  const handleInstallConfirm = useCallback(async () => {
    ...
    try {
      setInstallResult(
        await designContractApi.installContract(projectId, buildParams()),
      );
      setVerification(await designContractApi.verifyProjectSystem(projectId));
      toast.success(
        t("designContract.installed", { defaultValue: "Installed" }),
      );
    } catch (e) {
      toast.error(String(e));
    } finally {
  ...
  const handleImportFile = useCallback(async () => {
    ...
    try {
      ...
      if (r.quality.level === "needs_review")
        toast.warning(
          `导入内容需复核：缺少 ${r.quality.missingSections.join("、")}`,
        );
    } catch (e) {
      toast.error(String(e));
    } finally {
  ...
  const prototypeIndex =
    packageDetail?.components.pageTemplates?.indexOf(selectedPrototype ?? "") ??
    0;
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">
            {t("designContract.title", { defaultValue: "Design Contract" })}
          </h3>
        </div>
        {selectedTemplate && (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <Label className="text-[10px] font-medium text-muted-foreground uppercase">
            {t("designContract.name", { defaultValue: "Contract Name" })}
          </Label>
          <Input
        ...
        <div className="space-y-1">
          <Label className="text-[10px] font-medium text-muted-foreground uppercase">
            {t("designContract.description", { defaultValue: "Description" })}
          </Label>
          <Input
            className="h-8 text-xs"
            value={contractDesc}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4 h-8">
          <TabsTrigger value="templates" className="text-[10px] h-6">
            <Paintbrush className="h-3 w-3 mr-1" />
            {t("designContract.templates", { defaultValue: "Templates" })}
          </TabsTrigger>
          <TabsTrigger value="tokens" className="text-[10px] h-6">
            <Palette className="h-3 w-3 mr-1" />
            {t("designContract.tokens", { defaultValue: "Tokens" })}
          </TabsTrigger>
          <TabsTrigger value="preview" className="text-[10px] h-6">
            <Eye className="h-3 w-3 mr-1" />
            {t("designContract.preview", { defaultValue: "Preview" })}
          </TabsTrigger>
          <TabsTrigger value="import" className="text-[10px] h-6">
            <Upload className="h-3 w-3 mr-1" />
            {t("designContract.import", { defaultValue: "Import" })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="templates" className="mt-3">
          ...
          {loading && (
            <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
              {LoadIcon}
              {t("designContract.loadingTemplate", {
              ...
              <CardContent className="p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold">
                    {packageDetail.package.name} · 原型能力
                  </span>
                  <Badge variant="outline">
                    {packageDetail.package.licenseId}
                  </Badge>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {packageDetail.components.pageTemplates?.map((page) => (
                      <Button
                        key={page}
                        size="sm"
                        variant={
                          selectedPrototype === page ? "secondary" : "outline"
                        }
                        onClick={() => setSelectedPrototype(page)}
                ...
                <div className="grid grid-cols-2 gap-3 text-[10px]">
                  <div>
                    ...
                    <p>{packageDetail.components.components?.join(" · ")}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">主题/响应式</span>
                    <p>
                      {packageDetail.responsive.modes?.join(" / ")} ·{" "}
                      {packageDetail.responsive.rules?.join("；")}
                    </p>
                  </div>
                </div>
                <div className="rounded-md border bg-background p-3">
                  <span className="text-[10px] text-muted-foreground">
                    静态原型预览 · {selectedPrototype ?? "选择页面模板"}
                  </span>
                  <PrototypePreview
                    page={selectedPrototype}
                    components={packageDetail.components.components}
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <p className="text-[10px] text-muted-foreground">
                      {packageDetail.package.applicableScenarios.join(" / ")}
                    </p>
                    <Button
                      size="sm"
                      className="text-xs"
                ...
                <p className="text-[10px] text-muted-foreground">
                  {packageDetail.accessibility.replace(/^# .*\n+/, "")}
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="tokens" className="mt-3 space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-[10px] font-semibold uppercase text-muted-foreground">
              {t("designContract.colors", { defaultValue: "Colors" })}
            </Label>
            <div className="grid grid-cols-2 gap-2">
              {COLOR_FIELDS.map(({ key, label }) => (
                <div key={key} className="flex items-center gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor: (colors?.[key] as string) ?? "#ccc",
                    }}
                  />
          ...
          <div className="space-y-2">
            <Label className="text-[10px] font-semibold uppercase text-muted-foreground">
              {t("designContract.typography", { defaultValue: "Typography" })}
            </Label>
          ...
          <div className="flex items-center gap-3">
            <Label className="text-[10px] font-semibold uppercase text-muted-foreground">
              {t("designContract.spacing", {
                defaultValue: "Spacing Base (px)",
              })}
            </Label>
            <Input
              type="number"
              ...
              value={spacingBase}
              onChange={(e) => setSpacingBase(Number(e.target.value) || 4)}
              min={1}
              max={16}
            />
          </div>
          ...
          {previewMd ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  {t("designContract.copy", { defaultValue: "Copy" })}
                </Button>
              </div>
              <pre className="text-[10px] leading-relaxed bg-muted/50 border border-border/50 rounded-lg p-3 overflow-x-auto max-h-[400px] overflow-y-auto whitespace-pre-wrap break-words font-mono">
                {previewMd}
              </pre>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-2 py-8 text-muted-foreground">
              ...
              <span className="text-xs">
                {t("designContract.noPreview", {
                  defaultValue: "Click Preview to generate DESIGN.md",
                })}
              </span>
            </div>
          )}
        ...
        <TabsContent value="import" className="mt-3 space-y-4">
          <div className="space-y-2">
            ...
            >
              ...
              {t("designContract.pickFile", {
                defaultValue: "Pick DESIGN.md file...",
              })}
            </Button>
          </div>
          <div className="space-y-2">
            <Label className="text-[10px] font-semibold uppercase text-muted-foreground">
              {t("designContract.importUrl", {
                defaultValue: "导入可信远程设计规范",
              })}
            </Label>
            <div className="flex gap-1">
              {(["github", "shadcn"] as const).map((kind) => (
                <Button
                  key={kind}
                  size="sm"
                  variant={remoteImportKind === kind ? "secondary" : "ghost"}
                  className="h-6 px-2 text-[10px]"
                  onClick={() => setRemoteImportKind(kind)}
                >
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder={
                  remoteImportKind === "github"
                    ? "https://raw.githubusercontent.com/.../DESIGN.md"
                    : "https://ui.shadcn.com/..."
                }
              />
              <Button
      ...
      <div className="flex flex-wrap gap-2 border-t border-border/50 pt-3">
        ...
        >
          ...
          {t("designContract.copyBtn", { defaultValue: "Copy" })}
        </Button>
        <Button
          size="sm"
          onClick={handleExportPreview}
          disabled={exporting || !contractName}
          className="text-xs"
        >
          {exporting ? LoadIcon : <Download className="h-3.5 w-3.5 mr-1" />}
          {t("designContract.exportOverwriteBtn", {
            defaultValue: "覆盖导出 DESIGN.md",
          })}
        </Button>
        <Button
          size="sm"
        ...
        >
          ...
          {t("designContract.installBtn", {
            defaultValue: "Install to Project",
          })}
        </Button>
        <Button
          size="sm"
        ...
        <Card className="border-emerald-500/30 bg-emerald-500/5">
          <CardHeader className="pb-2 pt-3 px-3">
            <CardTitle className="text-xs font-semibold text-emerald-700 dark:text-emerald-400 flex items-center gap-2">
              ...
              {t("designContract.installResult", { defaultValue: "Installed" })}
              : {installResult.filesCreated.length}{" "}
              {t("designContract.filesCreated", {
                defaultValue: "files created",
              })}
              {installResult.filesSkipped.length > 0 &&
                `, ${installResult.filesSkipped.length} ${t("designContract.filesSkipped", { defaultValue: "skipped" })}`}
            </CardTitle>
          </CardHeader>
          <CardContent className="px-3 pb-3 pt-0 space-y-0.5">
            ...
            {installResult.filesSkipped.map((f) => (
              ...
              >
                ~ {f} ({t("designContract.exists", { defaultValue: "exists" })})
              </div>
            ))}
          </CardContent>
        </Card>
