...
import { Trash2, ExternalLink, Plus, Power, PowerOff } from "lucide-react";
...
interface RepoManagerPanelProps {
  ...
  onToggle: (owner: string, name: string, enabled: boolean) => Promise<void>;
  onClose: () => void;
}
              ...
              >
                ...
                <div className="flex gap-1.5">
                  <Button
                    ...
                    type="button"
                    onClick={() =>
                      onToggle(repo.owner, repo.name, !repo.enabled)
                    }
                    title={
                      repo.enabled
                        ? t("skills.repo.disable", { defaultValue: "禁用" })
                        : t("skills.repo.enable", { defaultValue: "启用" })
                    }
                    className={
                      repo.enabled
                        ? "hover:text-amber-500 hover:bg-black/5 dark:hover:bg-white/5"
