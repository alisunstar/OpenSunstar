...
/**
 * 静默安装 ClawHub 技能
 * 执行 npx clawhub@latest install <slug>，成功后刷新已安装列表
 */
export function useInstallClawHubSkill() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (slug: string) => skillsApi.installClawHubSkill(slug),
    onSuccess: () => {
      // ClawHub CLI 安装后技能可能出现在 SSOT 目录，刷新已安装列表
 ...
 * 启用/禁用 Skill 仓库
 */
export function useToggleSkillRepo() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      owner,
      ...
      name: string;
      enabled: boolean;
    }) => skillsApi.toggleRepo(owner, name, enabled),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["skills", "repos"] });
      queryClient.invalidateQueries({ queryKey: ["skills", "discoverable"] });
    },
 ...
 */
export function useSearchClawHub(query: string, limit: number) {
  return useQuery({
    queryKey: ["skills", "clawhub", query, limit],
    queryFn: () => skillsApi.searchClawHub(query, limit),
    enabled: query.length >= 2,
    staleTime: 5 * 60 * 1000,
    placeholderData: keepPreviousData,
  });
