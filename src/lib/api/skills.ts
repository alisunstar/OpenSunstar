...
export interface ClawHubSearchResult {
  skills: ClawHubDiscoverableSkill[];
  query: string;
}

/** ClawHub 技能统计（星标/下载/安装量） */
...
export const skillsApi = {
  ...
  async batchToggleApp(
    ...
    return await invoke("batch_toggle_skill_app", { ids, app, enabled });
  },
  ...
  async resyncAll(): Promise<number> {
    return await invoke("resync_all_skills");
  },
  ...
  async installClawHubSkill(slug: string): Promise<void> {
    return await invoke("install_clawhub_skill", { slug });
  },
  ...
  async toggleRepo(
    ...
    return await invoke("toggle_skill_repo", { owner, name, enabled });
  },
