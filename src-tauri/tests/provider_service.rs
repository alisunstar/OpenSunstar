...
"#;
    ...
    let auth_after_switch: serde_json::Value =
        read_json_file(&open_sunstar_lib::get_codex_auth_path()).expect("read auth after switch");
    assert_eq!(
    ...
    let auth_after_takeover: serde_json::Value =
        read_json_file(&open_sunstar_lib::get_codex_auth_path()).expect("read auth after takeover");
    assert_eq!(
    ...
    let restored_auth: serde_json::Value =
        read_json_file(&open_sunstar_lib::get_codex_auth_path()).expect("read restored auth");
    assert_eq!(
        restored_auth, oauth_auth,
        "disabling takeover should restore without replacing OAuth auth.json"
    );

    let restored_config = std::fs::read_to_string(open_sunstar_lib::get_codex_config_path())
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.OpenSunstar/settings.json
