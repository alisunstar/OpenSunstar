...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".OpenSunstar").join("settings.json")
}
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = open_sunstar_lib::get_claude_settings_path();
    ...
    assert!(
        db_path.exists(),
        "switching provider should persist to OpenSunstar.db"
    );
