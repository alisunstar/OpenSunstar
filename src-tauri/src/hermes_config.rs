...
///
/// Input (from OpenSunstar UI / database):
/// ```json
...
/// `providers:` dict. OpenSunstar treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by OpenSunstar)
...
/// keeping OpenSunstar free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay OpenSunstar must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a OpenSunstar edit to an unrelated field would silently
...
// OpenSunstar fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `OPEN_SUNSTAR_TEST_HOME` to avoid interfering with
    ...
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        ...
        std::env::set_var("OPEN_SUNSTAR_TEST_HOME", tmp.path());
        let result = test_fn();
        match old_test_home {
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later OpenSunstar edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a OpenSunstar toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
