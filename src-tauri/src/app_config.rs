...
impl SkillApps {
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::ClaudeDesktop => {} // Claude Desktop 3P profiles don't use OpenSunstar skill sync
        }
    }
...
/// 未管理的 Skill（在应用目录中发现但未被 OpenSunstar 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub struct McpRoot {
    ...
    /// Grok Build MCP servers are projected to `~/.grok/config.toml`.
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
...
impl MultiAppConfig {
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到 ~/.OpenSunstar/config.json.bak，再写入新内容
        if config_path.exists() {
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::GrokBuild => continue, // Grok Build has no legacy per-app store
