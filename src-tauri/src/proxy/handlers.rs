...
/// catalog with a top-level `models` field.  Return the OpenSunstar–managed model
...
/// OpenSunstar–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_open_sunstar_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to OpenSunstar catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 OpenSunstar 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a OpenSunstar limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "OpenSunstar local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "OpenSunstar_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "OpenSunstar_timeout",
        ProxyError::NoAvailableProvider => "OpenSunstar_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "OpenSunstar_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "OpenSunstar_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "OpenSunstar_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "OpenSunstar_provider_unhealthy",
        ProxyError::ConfigError(_) => "OpenSunstar_config_error",
        ProxyError::TransformError(_) => "OpenSunstar_transform_error",
        ProxyError::InvalidRequest(_) => "OpenSunstar_invalid_request",
        ProxyError::AuthError(_) => "OpenSunstar_auth_error",
        ProxyError::UpstreamError { .. } => "OpenSunstar_upstream_error",
        ProxyError::DatabaseError(_) => "OpenSunstar_database_error",
        ProxyError::Internal(_) => "OpenSunstar_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "OpenSunstar_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("OpenSunstar local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "OpenSunstar_forward_failed");
