...
const OPEN_SUNSTAR_SQL_EXPORT_HEADER: &str = "-- OpenSunstar SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_open_sunstar_sql_export(sql_content)?;
    ...
    fn validate_open_sunstar_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(OPEN_SUNSTAR_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 OpenSunstar 导出的 SQL 备份文件。",
            "Only SQL backups exported by OpenSunstar are supported.",
        ))
    }
