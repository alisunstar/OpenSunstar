...
use crate::codex_config::{
    get_codex_config_dir, read_codex_config_text, OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID,
};
...
const LEGACY_OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID: &str = "OpenSunstar";
...
const OPEN_SUNSTAR_LEGACY_CODEX_MODEL_PROVIDER_IDS: &[&str] = &[
    LEGACY_OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID,
    "aicodemirror",
    "aicoding",
    "aigocode",
    "aihubmix",
    ...
    if source_provider_ids.is_empty() {
        crate::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
                ...
                target_provider_id: OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID.to_string(),
...
fn collect_source_model_provider_ids(db: &Database) -> Result<BTreeSet<String>, AppError> {
    ...
    for provider in providers.values() {
        ...
        insert_known_open_sunstar_legacy_source_id(&mut ids, &provider.id);
        ...
        for provider_id in trusted_legacy_codex_model_provider_ids_from_config(config_text) {
            insert_known_open_sunstar_legacy_source_id(&mut ids, &provider_id);
        }
...
fn insert_known_open_sunstar_legacy_source_id(ids: &mut BTreeSet<String>, provider_id: &str) {
    let trimmed = provider_id.trim();
    if is_known_open_sunstar_legacy_codex_model_provider_id(trimmed) {
...
fn is_known_open_sunstar_legacy_codex_model_provider_id(provider_id: &str) -> bool {
    OPEN_SUNSTAR_LEGACY_CODEX_MODEL_PROVIDER_IDS
        .iter()
...
fn legacy_codex_model_provider_id_from_normalized_config(config_text: &str) -> Option<String> {
    ...
    let provider_id = doc
        ...
        .map(str::trim)?;
    if provider_id != OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID
        && provider_id != LEGACY_OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID
    {
        return None;
    }
...
fn normalized_legacy_codex_provider_name(name: &str) -> Option<&'static str> {
    if is_known_open_sunstar_legacy_codex_model_provider_id(name) {
        return OPEN_SUNSTAR_LEGACY_CODEX_MODEL_PROVIDER_IDS
            .iter()
            .copied()
...
fn insert_trusted_legacy_config_model_provider_id(
    ...
    if provider_id.is_empty()
        || !is_known_open_sunstar_legacy_codex_model_provider_id(provider_id)
...
fn migrate_provider_config_template_to_custom(
    ...
    let custom_table_exists =
        config_defines_model_provider(&doc, OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(source_provider_id) = source_provider_id_to_move {
        ...
        model_providers[OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID] = provider_table;
        changed = true;
    }
    ...
    {
        doc["model_provider"] = toml_edit::value(OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID);
        changed = true;
    }
    ...
    for profile_key in profile_keys {
        ...
        if references_legacy {
            profile_table.insert(
                "model_provider",
                toml_edit::value(OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID),
            );
            changed = true;
        }
    }
...
fn rewrite_codex_session_meta_line(
    ...
    payload.insert(
        ...
        Value::String(OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID.to_string()),
    );
...
fn migrate_codex_state_db_provider_bucket(
    ...
    values.push(OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID.to_string());
...
"#
            ...
            Provider::with_id(
                "legacy-OpenSunstar".to_string(),
                "Legacy OpenSunstar".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "OpenSunstar"

[model_providers.OpenSunstar]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "OpenSunstar", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"OpenSunstar\"}}\n",
        ...
        conn.execute_batch(
            ...
            INSERT INTO threads (id, model_provider) VALUES
                ...
                ('OpenSunstar-thread', 'OpenSunstar'),
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-OpenSunstar", "rightcode"])
        );
        ...
        assert!(open_sunstar_config
            .get("model_providers")
            .and_then(|value| value.get("OpenSunstar"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_open_sunstar_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"OpenSunstar\"\n\n[model_providers.OpenSunstar]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
        ...
        let ids = collect_source_model_provider_ids(&db).expect("collect ids");
        assert!(ids.contains("OpenSunstar"));
        assert!(ids.contains("aihubmix"));
        assert!(!ids.contains("generated-uuid"));
    }
...
"#
    ...
    #[test]
    fn migrates_legacy_open_sunstar_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-OpenSunstar".to_string(),
            "Legacy OpenSunstar".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-OpenSunstar".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-OpenSunstar", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
