...
pub enum SkillStorageLocation {
    /// OpenSunstar 管理目录 (~/.OpenSunstar/skills/)
    #[default]
    #[serde(alias = "cc_switch", alias = "open_mapping")]
    OpenSunstar,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.OpenSunstar/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::OpenSunstar => get_app_config_dir().join("skills"),
        ...
        if !dest.exists() {
            ...
            let audit_result = crate::audit::scan_dir(
                ...
                &crate::audit::AuditContext {
                    source: crate::audit::AuditSource::Install {
                        owner: skill.repo_owner.clone(),
            ...
            if audit_result.should_block() {
                let report = crate::audit::AuditReport::from(&audit_result);
                if let Ok(json) = serde_json::to_string_pretty(&report) {
                    log::error!("Skill 安全审计阻断:\n{json}");
                }
                let _ = fs::remove_dir_all(&temp_dir);
                let _ = fs::remove_dir_all(&dest);
                return Err(anyhow!(format_skill_error(
                    "SECURITY_AUDIT_BLOCKED",
                    &[
                        ("owner", &skill.repo_owner),
                        ("name", &skill.repo_name),
                        ("findings", &audit_result.total_findings().to_string()),
                    ],
                    None,
                )));
            }
    ...
    pub async fn update_skill(&self, db: &Arc<Database>, skill_id: &str) -> Result<InstalledSkill> {
        ...
        let audit_result = crate::audit::scan_dir(
            &source,
            &crate::audit::AuditContext {
                source: crate::audit::AuditSource::Update {
                    owner: owner.clone(),
                    repo: name.clone(),
    ...
    pub fn batch_toggle_app(
        ...
        for id in ids {
            match db.get_installed_skill(id) {
                Ok(Some(mut skill)) => {
                    // 如果状态已一致则跳过
                    if skill.apps.is_enabled_for(app) == enabled {
                        success_count += 1;
                        continue;
                    }
                    ...
                    if let Err(e) = sync_result {
                        log::warn!("Batch toggle: sync failed for {}: {e}", skill.name);
                        continue;
                    }

                    if let Err(e) = db.update_skill_apps(id, &skill.apps) {
                        log::warn!("Batch toggle: DB update failed for {}: {e}", skill.name);
                        continue;
                    }
                ...
                Ok(None) => {
                    log::warn!("Batch toggle: skill not found: {id}");
                }
                Err(e) => {
                    log::warn!("Batch toggle: error loading {id}: {e}");
                }
            }
        }

        log::info!(
            "Batch toggle {:?} → {}: {success_count}/{} skills succeeded",
            app,
            enabled,
            ids.len()
        );
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 OpenSunstar 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "OpenSunstar".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 OpenSunstar 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "OpenSunstar".to_string()));
    ...
    /// 查找仓库中所有包含 SKILL.md 的目录（深度最多 5 层）
    ///
    /// 用于 skills.sh 安装最终回退：当目录名与 skill_id 不匹配时，
    /// 扫描仓库中所有 SKILL.md 所在位置。
    fn find_all_skill_dirs(root: &Path) -> Vec<PathBuf> {
    ...
    /// 2. 仅持有安装名 `foo`，需要在仓库中递归查找真实目录（名称精确匹配）；
    /// 3. 仓库根目录本身就是 skill，此时回退到解压根目录；
    /// 4. skills.sh 的 skill_id 与仓库内实际目录名不一致时，
    ///    扫描仓库中所有 SKILL.md 目录，优先按名称部分匹配，否则取第一个。
    fn resolve_skill_source_dir(root: &Path, raw_directory: &str) -> Option<PathBuf> {
        ...
        // 策略4：扫描仓库中所有 SKILL.md 目录，尝试部分名称匹配或取首个
        let all_skill_dirs = Self::find_all_skill_dirs(root);
        if !all_skill_dirs.is_empty() {
            // 优先查找目录名与 target_name 有部分匹配的（如 "design" 匹配 "stitch-design"）
            let target_lower = target_name.to_lowercase();
            let target_parts: Vec<&str> = target_lower.split(['-', '_', ' ']).collect();

            for dir in &all_skill_dirs {
                let dir_name = dir
                    .file_name()
                    .map(|n| n.to_string_lossy().to_lowercase())
                    .unwrap_or_default();
                // 名称包含关系：目录名包含 target 或 target 包含目录名
                if dir_name.contains(&target_lower) || target_lower.contains(&dir_name) {
                    log::info!(
                        "Skill directory '{}' resolved by partial name match: {}",
                        target_name,
                        dir.display()
                    );
                    return Some(dir.clone());
                }
                // 按词段匹配：至少有一个词段出现在目录名中
                for part in &target_parts {
                    if part.len() > 2 && dir_name.contains(part) {
                        log::info!(
                            "Skill directory '{}' resolved by segment match '{}': {}",
                            target_name,
                            part,
                            dir.display()
                        );
            ...
            // 没有名称匹配，取第一个 SKILL.md 目录
            log::warn!(
                "Skill directory '{}' not found by name, using first SKILL.md found: {} (total {} found)",
                target_name,
                all_skill_dirs[0].display(),
                all_skill_dirs.len()
            );
            return Some(all_skill_dirs[0].clone());
        }
    ...
    pub async fn search_clawhub(
        ...
        let resp = client
            ...
            .await?
            .error_for_status()?
            .json::<ClawHubApiResponse>()
            .await?;

        let skills = resp
            .results
            .into_iter()
            .map(|r| ClawHubDiscoverableSkill {
                slug: r.slug,
                display_name: r.display_name.unwrap_or_default(),
                summary: r.summary.unwrap_or_default(),
                owner_handle: r
                    .owner_handle
                    .or_else(|| r.owner.and_then(|o| o.handle))
                    .unwrap_or_default(),
                stars: None,
                downloads: None,
            })
                            ...
                            Ok(r2) => match r2.json::<ClawHubDetailResponse>().await {
                                Ok(detail) => ClawHubSkillStats {
                                    ...
                                    downloads: detail
                                        ...
                                        .stats
                                        .as_ref()
                                        .and_then(|s| s.downloads),
                                    installs: detail
                                        .skill
                                        .stats
                                        .as_ref()
                                        .and_then(|s| s.installs_all_time),
                                },
                                Err(_) => ClawHubSkillStats {
                                    slug: slug.clone(),
                                    stars: None,
                                    downloads: None,
                                    installs: None,
...
struct ClawHubApiResponse {
    pub results: Vec<ClawHubApiResult>,
}

/// ClawHub API 原始搜索结果条目
#[derive(Debug, Clone, Deserialize)]
struct ClawHubApiResult {
    #[allow(dead_code)]
...
struct ClawHubDetailResponse {
    pub skill: ClawHubDetailSkill,
}

/// ClawHub API 详情中的 skill 对象
#[derive(Debug, Clone, Deserialize)]
struct ClawHubDetailSkill {
    pub stats: Option<ClawHubDetailStats>,
}

/// ClawHub API 统计数据
#[derive(Debug, Clone, Deserialize)]
struct ClawHubDetailStats {
    pub downloads: Option<u64>,
...
pub struct ClawHubSearchResult {
    pub skills: Vec<ClawHubDiscoverableSkill>,
    pub query: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ClawHubDiscoverableSkill {
    pub slug: String,
    pub display_name: String,
    pub summary: String,
...
/// 从 SSOT 目录、旧版应用目录和所有本地应用目录自动导入技能到数据库。
///
/// 当数据库的 skills 表为空时调用，确保：
/// - 升级用户：从旧版目录或 SSOT 恢复技能
/// - 首次安装用户：扫描本地已安装的技能（如 ~/.claude/skills/）并同步
///
/// 扫描以下目录（按优先级）：
/// 1. `~/.OpenSunstar/skills/`（SSOT 目录）
/// 2. 旧版应用的 skills 目录（如果存在）
/// 3. 各本地应用目录（~/.claude/skills/、~/.codex/skills/ 等）
/// 4. `~/.agents/skills/`（共享目录）
///
/// 找到的技能目录会被复制到 SSOT 并注册到数据库中。
pub fn auto_import_ssot_skills(db: &Arc<Database>) -> Result<usize> {
    let ssot_dir = SkillService::get_ssot_dir()?;
    ...
    // 1. 始终扫描 SSOT 目录
    if ssot_dir.exists() {
        scan_dirs.push(ssot_dir.clone());
    }

    // 2. 如果旧版应用目录存在，也加入扫描
    if let Some(legacy_skills) = dirs::home_dir()
        .map(|h| h.join(".cc-switch").join("skills"))
        .filter(|p| p.exists())
    {
        scan_dirs.push(legacy_skills);
    }

    // 3. 扫描各本地应用目录（首次安装用户可能已有技能）
    for app in AppType::all() {
        if let Ok(app_dir) = SkillService::get_app_skills_dir(&app) {
            if app_dir.exists() {
                scan_dirs.push(app_dir);
            }
        }
    }

    // 4. 扫描共享目录 ~/.agents/skills/
    if let Some(agents_dir) = dirs::home_dir()
        .map(|h| h.join(".agents").join("skills"))
        .filter(|p| p.exists())
    {
    ...
    for dir in &scan_dirs {
        ...
        for entry in entries.flatten() {
            ...
            // 先发现的优先（SSOT 优先于 legacy）
            discovered.entry(dir_name).or_insert(path);
        }
    }
    ...
    // 确保 SSOT 目录存在
    if !ssot_dir.exists() {
        fs::create_dir_all(&ssot_dir).map_err(|e| {
            log::warn!("Failed to create SSOT directory: {e}");
            e
        })?;
    }

    // 获取数据库中已有的技能目录名，避免重复导入
    let existing_dirs: std::collections::HashSet<String> = db
    ...
    let mut count = 0;
    for (dir_name, source_path) in discovered {
        ...
        let id = format!("{dir_name}-auto-import");
        let content_hash = SkillService::compute_dir_hash(&ssot_path).ok();
