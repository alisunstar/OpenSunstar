...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/alisunstar/OpenSunstar/releases/latest",
            None::<String>,
        )
...
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>) -> Result<(), String> {
    ...
    let temp_dir = std::env::temp_dir();
    let script_file = temp_dir.join(format!("OpenSunstar_launcher_{}.sh", std::process::id()));
...
fn launch_windows_terminal(
    ...
    let bat_file = temp_dir.join(format!("OpenSunstar_claude_{}.bat", std::process::id()));
...
// shell 命令拼接辅助，重构中暂未接线，保留供后续终端集成使用
#[allow(dead_code)]
...
// shell 单引号转义辅助，重构中暂未接线，保留供后续终端集成使用
#[allow(dead_code)]
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("OpenSunstar_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/bin/bash
trap 'rm -f "{script_path}"' EXIT
echo "[OpenSunstar] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[OpenSunstar] Command exited. Press any key to close."
read -n 1 -s
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("OpenSunstar_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [OpenSunstar] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [OpenSunstar] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 OpenSunstar 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 OpenSunstar
