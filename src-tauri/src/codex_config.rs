...
pub const OPEN_SUNSTAR_CODEX_MODEL_PROVIDER_ID: &str = "custom";
pub const OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME: &str = "OpenSunstar-model-catalog.json";
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME)
}
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            doc["model_catalog_json"] = toml_edit::value(OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME);
        }
        None => {
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(false);
            if should_remove {
...
/// OpenSunstar–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// OpenSunstar–generated file (identified by filename
/// `OpenSunstar-model-catalog.json`). A user-managed external catalog file is
...
/// Given `config.toml` text, resolve the on-disk path of the OpenSunstar–owned
/// catalog file.
...
pub(crate) fn resolve_open_sunstar_catalog_path(
    config_text: &str,
    generated_path: &Path,
) -> Option<PathBuf> {
    ...
    let is_open_sunstar_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME);
    if !is_open_sunstar_owned {
        return None;
    }
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/OpenSunstar-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        ...
        assert!(resolve_open_sunstar_catalog_path("", &generated).is_none());
        assert!(
            resolve_open_sunstar_catalog_path("model = \"gpt-5\"", &generated).is_none(),
            "no model_catalog_json field should yield None"
        );
    }

    #[test]
    fn resolve_catalog_path_accepts_open_sunstar_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/OpenSunstar-model-catalog.json");
...
"#;
        assert!(
            resolve_open_sunstar_catalog_path(config, &generated).is_none(),
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as OpenSunstar would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\OpenSunstar-model-catalog.json");
        ...
        assert_eq!(
            written_path, OPEN_SUNSTAR_CODEX_MODEL_CATALOG_FILENAME,
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/OpenSunstar-model-catalog.json");
    ...
    #[test]
    fn set_catalog_json_none_removes_open_sunstar_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/OpenSunstar-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove OpenSunstar-owned field regardless of path format"
        );
    }
...
model_catalog_json = "OpenSunstar-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/OpenSunstar-model-catalog.json");
        let result = resolve_open_sunstar_catalog_path(config_text, &generated_path);
        assert_eq!(
            result,
            Some(generated_path),
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by OpenSunstar"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "OpenSunstar-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative OpenSunstar-owned field"
        );
    }
}
